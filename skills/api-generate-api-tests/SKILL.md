...
阶段 1 全量做一次。阶段 2 先圈定改动面、按改动点划出批次骨架（全量，一次），
批次一旦定下就贯穿全程：此后阶段 2 到阶段 6 整体按批次循环——每批先讲透这批的
改动逻辑和测试场景，用户确认后再做这批的 mock、造数、用例、执行，一批做完再看
下一批。这样用户是跟着一批一批走完整条流水线逐步吃透改动，而不是先啃完一整份
改动分析，再机械地按批次生成用例。

| 阶段 | 核心子 skill | vendor 适配（若有） | 产出 | 粒度 |
|---|---|---|---|---|
| 1 | 本 skill 收集输入 | — | `docs/00-context.md` | 全量 |
| 2 | `api-analyze-change-scenarios` | — | `docs/01-change-analysis.md`、`docs/batch<N>/02-scenarios.md` | 01 骨架全量一次、行为差异按批追加；场景每批一份 |
...
```
阶段 1 收集输入                        ── 全量，一次
阶段 2-a 圈定改动面 + 规划批次骨架       ── 全量，一次（只列改动点清单和批次划分，
                                          不展开行为差异，见「批次安排」）→ 暂停确认
  ┌── 对每个批次循环 ──────────────────────────────┐
  │ 阶段 2-b 本批改动点行为差异分析 + 场景设计       │
  │   行为差异追加进 01-change-analysis.md           │
  │   场景写入 docs/batch<N>/02-scenarios.md         │
  │   → 暂停确认（用户借此消化本批改动逻辑）          │
  │ 阶段 3 mock 方案（仅本批次场景）                 │
  │ 阶段 4 框架造数（仅本批次场景）                   │
  │ 阶段 5 用例生成（仅本批次场景）                   │
...
批次划分和逐批展开的规则见 `api-analyze-change-scenarios`。要点：**batch1 固定收纳
1–3 个能代表核心/基础能力的改动点，剩余改动点默认全部塞进 batch2**，改动点很多或
预判场景会爆量时才在骨架阶段切更多批次；批次一旦确认，行为差异分析、场景设计、
mock、造数、用例、执行都只做当前这一批，不因为"改动不大""想省几轮来回"就提前把
未确认批次的内容一起讲了——除非用户明确要求不分批或明确要求一次讲完所有批次。

**用户明确要求不分批时**退化为单批：全部改动点和场景归 batch1，阶段 2-b 不再逐批
展开，目录结构不变。除此之外不要自作主张跳过分批或多拆批次。
...
`docs/01-change-analysis.md` 顶层状态为 `待确认` 时，看状态头的**批次进度**行判断
续跑位置：骨架未确认就先确认骨架（此时还没有任何 `batch<N>/` 目录，属正常情况）；
骨架已确认但某批次标「待确认」，就看 `docs/batch<N>/02-scenarios.md`——没有或状态
是 `待确认`，就继续或等待该批的行为差异分析 + 场景设计。该文件状态改成 `已确认`
后，才进入该批的阶段 3。`01` 的顶层状态要等所有批次的场景都确认完才改成 `已确认`。

判断当前批次的方法：批次号最大的那个 `docs/batch<N>/` 目录就是进行中的批次；
它下面 `06-run-report.md` 已确认，说明该批次做完了，下一步是开新批次。开新批次前
不要提前建下一批的目录，也不要写一份汇总所有批次的 `docs/02-scenarios.md`。
